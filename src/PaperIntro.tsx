...
const PaperPresentation = () => {
  ...
  // Canvas Particle System - 3D Quantum Neural Network (Gentle)
  useEffect(() => {
    ...
    // State
    const particles: { x: number, y: number, z: number, connections: number[] }[] = [];
    const pulses: { p1: number, p2: number, progress: number, speed: number }[] = [];
    let mouseX = 0;
    let mouseY = 0;
    let targetRotY = 0;
    let currentRotX = 0;
    ...
    // Initialize Particles in a sphere/cloud
    for (let i = 0; i < particleCount; i++) {
        const theta = Math.random() * Math.PI * 2;
        const phi = Math.acos((Math.random() * 2) - 1);
        const r = 0.8 + Math.random() * 0.4; // Radius variation
    ...
    const project = (x: number, y: number, z: number) => {
        ...
        const x2d = (x * scale) * (Math.min(width, height) * 0.85) + width / 2;
        const y2d = (y * scale) * (Math.min(width, height) * 0.85) + height / 2;
        return { x: x2d, y: y2d, scale };
    };

    const rotate = (x: number, y: number, z: number, pitch: number, yaw: number) => {
        // Rotate Y (Yaw)
    ...
    const animate = () => {
        ...
        // Add mouse influence (gentle parallax)
        const mouseInfX = (mouseY - height/2) * mouseSensitivity;
        const mouseInfY = (mouseX - width/2) * mouseSensitivity;
        
        currentRotX += (mouseInfX - currentRotX) * 0.05;
        currentRotY += (targetRotY + mouseInfY - currentRotY) * 0.05;

        // Draw connections and pulses
        const projected = particles.map(p => {
            const r = rotate(p.x, p.y, p.z, currentRotX, currentRotY);
        ...
        if (Math.random() < 0.05) {
            ...
            if (particles[startIdx].connections.length > 0) {
                const endIdx = particles[startIdx].connections[Math.floor(Math.random() * particles[startIdx].connections.length)];
                pulses.push({
                    p1: startIdx,
                    p2: endIdx,
                    progress: 0,
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            for (const neighborIdx of particles[i].connections) {
                 // Optimization: only draw if i < neighborIdx to avoid double drawing
                 // But wait, our connections list might strictly be one-way or duplicate?
                 // In our init loop: j = i + 1, so we pushed j to i's connections. 
                 // We didn't push i to j's. So this loop only draws each connection once. Default.
                 // Wait, I only pushed to i's list. So i contains higher indices. Correct.
                 
                 const p2 = projected[neighborIdx];
                 
                 // Distance fading for depth effect
                 const depth = (p1.scale + p2.scale) / 2;
                 const opacity = Math.min(1, Math.max(0.1, depth - 0.4)); 
        ...
        for (let i = pulses.length - 1; i >= 0; i--) {
            ...
            const p1 = projected[pulse.p1];
            const p2 = projected[pulse.p2];
            ...
            const depth = (p1.scale + p2.scale) / 2;
            const size = 3 * depth;
            const alpha = 1 - Math.abs(pulse.progress - 0.5) * 2; // Fade in/out
        ...
        projected.forEach((p) => {
             ...
             const opacity = Math.max(0.1, (p.scale - 0.2) * 0.7); // Fade distant nodes

             ctx.beginPath();
             ctx.arc(p.x, p.y, size, 0, Math.PI * 2);
             ctx.fillStyle = `rgba(135, 206, 250, ${opacity})`; // Light Sky Blue
             ctx.fill();
        });
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800" style={{ fontFamily: '"Noto Sans TC", sans-serif' }}>
      ...
      <header ref={heroRef} className="relative bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white pt-24 pb-32 overflow-hidden">
        ...
        <canvas 
          ...
          style={{ zIndex: 1 }} 
        />
        {/* Deep Neural Network Background - SVG Removed for 3D Effect */}
